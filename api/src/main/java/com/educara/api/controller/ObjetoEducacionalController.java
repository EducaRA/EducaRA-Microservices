...
import com.educara.api.model.objetoEducacional.DadosDetalhamentoObjetoEducacional;
import com.educara.api.model.objetoEducacional.DadosListagemObjetoEducacional;
...
public class ObjetoEducacionalController {
    ...
    public ResponseEntity listar(@PageableDefault(size = 10, sort = {"nome"}) Pageable paginacao) {
            var page = repository.findAllByAtivoTrue(paginacao).map(DadosListagemObjetoEducacional::new);
    ...
    public ResponseEntity detalhar(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(new DadosDetalhamentoObjetoEducacional(objetoEducacional));
    }
